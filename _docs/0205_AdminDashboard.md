...
| 30/06/2026 | **Correção do Bug de Mais Faltas:** Atualizado o frontend (`alunos/page.tsx`) para usar fallback local híbrido resiliente caso a RPC esteja desatualizada, garantindo exibição de ausências de logs_embarque de imediato. Criada migração SQL contendo a atualização da RPC `get_dashboard_metrics` para unificação correta de faltas. |
