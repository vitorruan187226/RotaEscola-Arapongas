...
| 25/06/2026 | **Fluxo de Re-cadastro de Carteirinhas Expiradas:** Quando a carteirinha expira, o grid de botões de ação do estudante é substituído por um único botão grande "Realizar Re-cadastro". Ele abre o modal `RecadastroModal` pré-preenchido com os dados atuais do aluno (passíveis de alteração) e permite atualizar ou enviar novos documentos com preservação dos arquivos anteriores válidos. O envio altera o status do aluno para `'aguardando'`, status da carteirinha para `'Pendente'`, remove a designação de rota e limpa os alertas de expiração do banco de dados. |
| 25/06/2026 | **Simulador Temporal de Alta Precisão (Minutos):** Conversão do widget `CalendarioFicticioWidget` para usar `datetime-local` com precisão de minutos. Adicionados botões de atalho rápido de `+1 Minuto` e `+5 Minutos` para facilitar a validação em tempo real da expiração reativa de 1 minuto sem necessidade de espera real. |
| 25/06/2026 | **Remoção de Ferramentas de Teste:** Remoção completa do widget de simulação temporal `CalendarioFicticioWidget` e simplificação do helper `getSimulatedDate()` para retornar estritamente a data real do sistema, restabelecendo a fidelidade operacional após a conclusão dos testes de expiração. |
