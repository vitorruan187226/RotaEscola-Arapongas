...
| 13/07/2026 | **Gestão de Capacidade da Frota (RPC):** Criada a RPC `get_rotas_capacidade` que unifica os limites da coluna `capacidade` de `veiculos` com as inscrições ativas de `alunos`. Isso permite o bloqueio em tempo real de novas vinculações no momento da aprovação do aluno, caso o veículo da rota selecionada já tenha atingido 100% da lotação máxima, além de disparar avisos na interface da secretaria ao bater 90% da capacidade. |
