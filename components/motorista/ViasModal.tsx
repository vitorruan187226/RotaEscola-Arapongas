...
import { Map, X, CheckCircle2, Send } from 'lucide-react';
import { createClient } from '../../utils/supabase/client';

interface ViasModalProps {
  isOpen: boolean;
  onClose: () => void;
  rotaNome: string;
...
export function ViasModal({ isOpen, onClose, rotaNome, onToast }: ViasModalProps) {
  ...
  const handleEnviarVias = async () => {
    ...
    try {
      ...
      onToast(err.message || 'Falha ao enviar alerta de via à central.', 'error');
    } finally {
